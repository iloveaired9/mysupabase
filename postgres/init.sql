...
-- Step 3: 게시글 데이터베이스 스키마 및 샘플 데이터

-- ============================================
-- 0. PostgREST 역할 설정
-- ============================================

-- anon 역할 생성 (PostgREST 기본 인증 없는 사용자)
...
CREATE INDEX idx_posts_comments ON posts(comments DESC);
...
-- 뉴스 관련 (10개)
('세계 경제 전문가들 내년 경기 회복 전망 제시', '국제경제기구가 내년 경기 회복을 전망했습니다.', 2, 7, 'https://bbc.com', 'economist', 892, 234, 5432, false, NOW() - INTERVAL '4 hours'),
...
('국제 무역 분쟁의 해결책 모색', '글로벌 경제 불균형을 어떻게 해결할 것인가?', 2, 7, 'https://bbc.com', 'trade_expert', 1234, 198, 5432, false, NOW() - INTERVAL '20 hours'),
('에너지 위기 시대의 새로운 해법', '재생 에너지의 확대가 전 세계적으로 가속화되고 있습니다.', 2, 7, 'https://bbc.com', 'energy_analyst', 2345, 423, 10987, true, NOW() - INTERVAL '26 hours'),
('팬데믹 이후 보건 시스템의 변화', '전 세계 보건 시스템이 새로운 방향으로 변모하고 있습니다.', 2, 7, 'https://bbc.com', 'health_expert', 1456, 267, 6543, false, NOW() - INTERVAL '32 hours'),
...
('국제 분쟁과 평화 협상', '현안이 되고 있는 국제 분쟁의 해법을 찾습니다.', 2, 7, 'https://bbc.com', 'diplomat', 1654, 298, 7123, true, NOW() - INTERVAL '50 hours'),
...
('국제 스포츠 정치의 현황', '스포츠와 정치의 얽힌 관계를 분석합니다.', 4, 7, 'https://bbc.com', 'political_analyst', 1565, 287, 7123, false, NOW() - INTERVAL '58 hours'),
...
-- ============================================
-- 7. PostgREST RPC 함수들 (자동 API 생성용)
-- ============================================
...
-- 출처별 게시글 수 반환 (게시글 수 많은 순)
CREATE OR REPLACE FUNCTION get_sources_with_count()
RETURNS TABLE(
    id INTEGER,
    name VARCHAR,
    icon_emoji VARCHAR,
...
GRANT EXECUTE ON FUNCTION get_sources_with_count() TO anon;
